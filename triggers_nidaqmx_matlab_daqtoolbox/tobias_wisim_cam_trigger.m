...
P = struct;
P.wisim_fps = 18;
P.duration_trial_secs = 15 * 60;
P.duration_baseline_secs = 30;
P.period_arduino_clk_secs = 5;
P.offset_visual_secs = 6;
P.n_visual = 0;
P.period_visual_secs = 5;  % number of phase steps in visual stim matlab script: 13
P.duration_visual_secs = 0;  % only controls pulse duration and not stimulus duration
P.period_whisker_secs = 60;
P.duration_whisker_secs = 0.5;
...
disp(['No. of WiSIM cam frames: ' num2str(P.wisim_fps * P.duration_trial_secs)]);

%% 30 min rec: 10 min Brush + visual, then 10 min spont, then 10 min brush & visual again
P = struct;
P.wisim_fps = 10;
P.duration_trial_secs = 30 * 60;
P.duration_baseline_secs = 30;
P.period_arduino_clk_secs = 5;
P.offset_visual_secs = 6;
P.n_visual = 13;
...
P.duration_visual_secs = 1;  % only controls pulse duration and not stimulus duration
P.period_whisker_secs = (P.n_visual + 1) * P.period_visual_secs;
P.nostim_t0_t1 = P.duration_baseline_secs + [P.period_whisker_secs * 8, P.period_whisker_secs * 16];  % start and end of no-stimulus period in seconds
P.duration_whisker_secs = 0.5;
...
%% 30 min rec, 18fps, for revision 1: 10 min Brush + visual, then 10 min spont, then 10 min brush & visual again
P = struct;
P.wisim_fps = 18;
P.duration_trial_secs = 30 * 60;
P.duration_baseline_secs = 30;
...
do_table(1 : P.period_arduino_clk_secs * P.wisim_fps : end, P.line_arduino_clk + 1) = 1;
nostim_t0_tick = P.nostim_t0_t1(1) * P.wisim_fps;
nostim_t1_tick = P.nostim_t0_t1(2) * P.wisim_fps;
...
do_table(nostim_t0_tick : nostim_t1_tick, P.line_whisker + 1) = 0;
...
do_table(nostim_t0_tick : nostim_t1_tick,  P.line_visual + 1) = 0;
%do_table(((P.duration_baseline_secs + P.offset_visual_secs) * P.wisim_fps) : (P.period_visual_secs  * P.wisim_fps) : end, P.line_visual + 1) = 1;
...
do_ch = addDigitalChannel(do_sess, P.daq_dev, P.do_port, 'OutputOnly');
...
%% For verification experiments: brush + led + air puff
P = struct;
P.wisim_fps = 141.5;
P.duration_trial_secs = 10 * 60;
P.duration_baseline_secs = 30;
...
P.period_airpuff_secs = 60;
P.offset_airpuff_secs = 30;

P.daq_dev = 'PXI1slot4';
P.do_port = 'Port0/Line4:6'; % 4 = airpuff, 5 = whisker stim
%P.daq_dev = 'Dev4';

% these are 0-based indices into the lines in P.do_port
P.line_arduino_clk = 2;  % this goes to wisim_clk input on arduino and causes LED to blink
P.line_ir_led = nan;  % not using this anymore
P.line_whisker = 1;
P.line_wisim_led = nan;
P.line_airpuff = 0;
%P.line_visual = 5; % --> jump wire to port0:4 for other matlab

%%
do_table = zeros(P.wisim_fps * P.duration_trial_secs, 3);
...
airpuff_dframes = 1;
airpuff_offsets = (P.duration_baseline_secs + P.offset_airpuff_secs : P.period_airpuff_secs : (P.duration_trial_secs)) * P.wisim_fps;
for i = 1:numel(airpuff_offsets)
    do_table(airpuff_offsets(i) : airpuff_offsets(i) + whisker_dframes, P.line_airpuff + 1) = 1;
end
%do_table(((P.duration_baseline_secs + P.offset_visual_secs) * P.wisim_fps) : (P.period_visual_secs  * P.wisim_fps) : end, P.line_visual + 1) = 1;
...
%% Verification: Clock from counter in 
do_sess = daq.createSession('ni');
do_ch = addDigitalChannel(do_sess, P.daq_dev, P.do_port, 'OutputOnly')
do_sess.IsContinuous = 1;
do_sess.Rate = P.wisim_fps;
do_sess.ExternalTriggerTimeout = 100;
%addClockConnection(do_sess, 'External', [P.daq_dev '/PFI7'], 'ScanClock')
%do_sess.addTriggerConnection('External', [P.daq_dev '/PFI7'], 'StartTrigger')
...
%% Verification: airpuff
% P.line_arduino_clk = 2;  % this goes to wisim_clk input on arduino and causes LED to blink
% P.line_whisker = 1;
% P.line_airpuff = 0;
outputSingleScan(do_sess, [1 0 0]);
pause(1);
outputSingleScan(do_sess, [0 0 0]);

%% Verification: whisker
% P.line_arduino_clk = 2;  % this goes to wisim_clk input on arduino and causes LED to blink
% P.line_whisker = 1;
% P.line_airpuff = 0;
outputSingleScan(do_sess, [0 1 0]);
pause(0.3);
outputSingleScan(do_sess, [0 0 0]);

%% Verification: clk = ir led
% P.line_arduino_clk = 2;  % this goes to wisim_clk input on arduino and causes LED to blink
% P.line_whisker = 1;
% P.line_airpuff = 0;
outputSingleScan(do_sess, [0 0 1]);
pause(0.3);
outputSingleScan(do_sess, [0 0 0]);
